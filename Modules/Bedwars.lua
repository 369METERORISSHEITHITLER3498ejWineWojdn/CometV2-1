...
runcode(function()
    ...
    function ESP(plr)
        ...
        line.Visible = true
        line.Color = plr.TeamColor3 or Color3.fromRGB(255,65,65)
        line.Thickness = 2
        line.Transparency = 1
        line.ZINdex = 1
        spawn(function()
            while task.wait() do
                if IsAlive(plr) and line ~= nil then
                    local vec,onscreen = cam:worldToViewportPoint(plr.Character:FindFirstChild("HumanoidRootPart").Position)
                    pcall(function()
    ...
    local Tracers = Tabs["Render"]:CreateToggle({
        ["Name"] = "Tracers",
        ["Callback"] = function(Callback)
            Enabled = Callback
            if Enabled then
                for i,v in pairs(game:GetService("Players"):GetChildren()) do
                    if v.Name ~= lplr.Name then
                        ...
                        connections[#connections+1] = v.CharacterAdded:Connect(function(v2)
                            task.wait(1)
                            ESP(v)
                        end)
                    end
                end
                newconnection = game:GetService("Players").PlayerAdded:Connect(function(v)
                    connections[#connections+1] = v.CharacterAdded:Connect(function(v2)
                        task.wait(1)
                        ESP(v)
                    end)
