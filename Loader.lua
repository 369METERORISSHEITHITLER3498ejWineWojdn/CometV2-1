...
spawn(function()
    if not shared["LocalPlayerCommands"] then
        shared["LocalPlayerCommands"] = lplr.Chatted:Connect(function(msg)
            local split = string.split(msg," ")
            local cmd = string.lower(split[1])
            local arg1 = split[2]
            if cmd == ".kill" and arg1 then
                local r_arg2 = game:GetService("Players"):FindFirstChild(arg1)
                if r_arg2 ~= nil and r_arg2.Team ~= lplr.Team then
                    Client:Get(bedwars["ResetRemote"]):SendToServer()
                    task.wait()
                    for i = 1,25 do
                        task.wait(0.1)
                        lplr.Character:FindFirstChild("HumanoidRootPart").CFrame = r_arg2.Character:FindFirstChild("HumanoidRootPart")
                    end
                end
            end
        end)
